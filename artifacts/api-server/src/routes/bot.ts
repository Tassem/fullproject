...
To cancel, send a new card message.`);
  ...
  if (parsed?.title) {
    ...
    const session: PendingCard = {
      ...
      aiPrompt: parsed.ai_prompt,
      aiStyle: parsed.ai_style,
    };
    pendingSessions.set(chatId, session);

    if (session.aiPrompt) {
      pendingSessions.delete(chatId);
      await generateAndSend(token, chatId, session, null, sendMsg);
      return;
    }
...
async function generateAndSend(
  ...
  if (session.aiPrompt) {
    await sendMsg(`⏳ Generating AI background...\n\n<i>"${session.aiPrompt.slice(0, 50)}${session.aiPrompt.length > 50 ? "..." : ""}"</i>`);
    
    const perm = await checkAiImagePermission(session.userId);
    if (!perm.allowed) {
      await sendMsg(`❌ <b>AI Generation Not Allowed</b>\n\n${perm.reason}`);
      return;
    }
    const aiResult = await deductAiImageCredits(session.userId, "telegram", session.aiPrompt);
    if (!aiResult.success) {
      await sendMsg(`❌ <b>AI Generation Failed</b>\n\n${aiResult.error}`);
      return;
    }
    ...
    try {
      ...
      const nanoRes = await fetch(`http://127.0.0.1:${port}/api/nanobanana/v1/images/generations`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt, n: 1, size: session.ratio === "16:9" ? "1920x1080" : session.ratio === "9:16" ? "1080x1920" : "1024x1024" })
      });
      ...
      if (nanoData.data && nanoData.data[0] && nanoData.data[0].url) {
         const imgRes = await fetch(nanoData.data[0].url);
         bgBuffer = Buffer.from(await imgRes.arrayBuffer());
      } else {
         throw new Error("No image generated by AI");
      }
    } catch (err: any) {
      console.error("[BOT] AI generation error:", err);
      await sendMsg("❌ AI generation failed: " + err.message);
      await refundAiImageCredits(session.userId, aiTxId);
      return;
    }
  } else {
  ...
  try {
    ...
    if (session.aiPrompt && aiTxId) await refundAiImageCredits(session.userId, aiTxId);
    return;
  }
