...
router.get("/", requireAuth, async (req, res) => {
  ...
  const items = await db.select().from(templatesTable).where(
    or(eq(templatesTable.userId, user.id), eq(templatesTable.isPublic, true), eq(templatesTable.isSystem, true))
  );
...
router.put("/:id", requireAuth, async (req, res) => {
  ...
  const [updated] = await db.update(templatesTable).set({
    ...
    watermark: watermark !== undefined ? watermark : existing.watermark,
    updatedAt: new Date(),
